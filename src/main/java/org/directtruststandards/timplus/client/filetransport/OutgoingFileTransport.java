...
{	
	...
	protected List<FileTransferStatusListener> fileTransferStatusListeners;
	...
	public OutgoingFileTransport(AbstractXMPPConnection con)
	{
		...
		this.fileTransferStatusListeners = new ArrayList<>();
	...
	public void addFileTransferStatusListener(FileTransferStatusListener listener)
	{
		fileTransferStatusListeners.add(listener);
	}
	...
	protected class Socks5ConnectionManager implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				updateStatusListeners(FileTransferState.INITIATOR_CANDIDATE_USED);
	...
	protected void executeIBBFallBack()
	{
		negotiateTransferExecutor.execute(new Runnable()
		{
			...
			public void run()
			{
				updateStatusListeners(FileTransferState.TRANSPORT_REPLACE);
	...
	protected void updateStatusListeners(FileTransferState status)
	{
		for (FileTransferStatusListener listener : fileTransferStatusListeners)
		{
			/*
			 * Don't let the listener disrupt the progress
			 */
			try
			{
